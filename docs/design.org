...
** Transforms
Here are the transforms those will support
*** Split
Powered directly by polyhedral compilation.
*** Tile
Powered directly by polyhedral compilation.
*** Reorder
*** Vectorize
The ~Vecotrize~ transform is powered by heuristic, we found it is hard to express in polyhedral compilation.

A straightforward example is converting the following code to X86 SIMD
...
First, lets get the equivalent IR representation:
...
The pre-requirements of Vectorize is that the stride of the forloop should be fixed, and all the operations in body support vectorization.

The vectorize period are as follows

1. vectorize all the operators
2. vectorize all the memory.
